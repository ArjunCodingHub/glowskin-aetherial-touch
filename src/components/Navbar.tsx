...
const Navbar = () => {
  ...
  return (
    <div className={cn(
      "fixed top-0 left-0 w-full z-50 transition-all duration-500 glassmorphism border-b",
      ...
      <div className="container mx-auto flex items-center justify-between px-4">
            ...
            {['home', 'treatment', 'product', 'about', 'contact'].map((item) => (
              <li key={item}>
                <button
                  ...
                  className="group relative text-gray-700 font-medium transition-colors hover:text-glow-purple"
                >
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden text-gray-700 transition-transform active:scale-90 duration-200"
      ...
      <div
        className={cn(
          "absolute top-full left-0 w-full bg-white/90 backdrop-blur-md shadow-md transition-all duration-500 ease-in-out overflow-hidden md:hidden",
          menuOpen ? "max-h-64 border-t py-4 opacity-100" : "max-h-0 opacity-0"
        )}
      >
        <ul className="flex flex-col space-y-4 px-6 py-2">
          {['home', 'treatment', 'product', 'about', 'contact'].map((item, index) => (
            <li key={item} 
